...
if(!function_exists('sparks_mikado_get_title')) {
    /**
     * Loads title area HTML
     */
    function sparks_mikado_get_title() {
        $id = sparks_mikado_get_page_id();

        extract(sparks_mikado_title_area_height());
        extract(sparks_mikado_title_area_background());
        ...
        if(($meta_temp = get_post_meta($id, "mkd_show_title_area_meta", true)) !== ""){
            ...
            $show_title_area = sparks_mikado_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
        }
        ...
        if(($meta_temp = get_post_meta($id, "mkd_title_area_type_meta", true)) !== ""){
            ...
            $type = sparks_mikado_options()->getOptionValue('title_area_type');
        }
        ...
        if(($meta_temp = get_post_meta($id, "mkd_title_area_enable_breadcrumbs_meta", true)) !== ""){
            ...
            $enable_breadcrumbs = sparks_mikado_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
        }
        ...
        $title_area_separator = false;
        //check if breadcrumbs are enabled on page first, then in options
        if(sparks_mikado_get_meta_field_intersect('title_area_separator') === 'yes') {
            $title_area_separator = true;
        }
        ...
        $parameters = array(
            ...
            'title_area_separator' => $title_area_separator,
        ...
        $parameters = apply_filters('sparks_mikado_title_area_height_params', $parameters);

        sparks_mikado_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('sparks_mikado_get_title_text')) {
    /**
     * Function that returns current page title text. Defines sparks_mikado_title_text filter
     ...
     * @see sparks_mikado_is_woocommerce_installed()
     */
    function sparks_mikado_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'sparks');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'sparks') . " " . get_the_author();
        }
        ...
        elseif (is_search()) {
            ...
            $title = esc_html__('Search Results', 'sparks');
        }

        //is single post page
        elseif (is_singular('post')) {
            if (sparks_mikado_options()->getOptionValue('blog_single_title_in_title_area') == "yes") {
                $title = get_the_title($id);
            } else {
                $title = esc_html__('Blog', 'sparks');
            }
        }
        ...
        elseif (is_404()) {
            ...
            if(sparks_mikado_options()->getOptionValue('404_title') != "") {
                //get it from options
                $title = sparks_mikado_options()->getOptionValue('404_title');
            } else {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'sparks');
            }
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(sparks_mikado_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
        ...
        elseif(sparks_mikado_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','sparks');
        }
        ...
        $title = apply_filters('sparks_mikado_title_text', $title);

        return $title;
    }
}

if(!function_exists('sparks_mikado_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see sparks_mikado_get_title_text()
     */
    function sparks_mikado_title_text() {
        echo sparks_mikado_get_title_text();
    }
}

if(!function_exists('sparks_mikado_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     *
     */
    function sparks_mikado_subtitle_text() {
        $id = sparks_mikado_get_page_id();
        $subtitle 	= '';
...
if(!function_exists('sparks_mikado_custom_breadcrumbs')) {
    /**
     ...
     * @see is_front_page()
     * @see is_category()
     * @see sparks_mikado_is_product_category()
     * @see get_search_query()
     */
    function sparks_mikado_custom_breadcrumbs() {
        global $post;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = sparks_mikado_get_page_id();
        $bread_style = "";
        ...
        $delimiter = '<span class="mkd-delimiter" '.sparks_mikado_get_inline_style($bread_style).'>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
        $home = get_bloginfo('name'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="mkd-current" '.sparks_mikado_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="mkd-breadcrumbs"><div class="mkd-breadcrumbs-inner"><a itemprop="url" '.sparks_mikado_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a itemprop="url" '.sparks_mikado_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="mkd-breadcrumbs"><div class="mkd-breadcrumbs-inner"><a itemprop="url" '.sparks_mikado_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            if(get_post_type() == 'portfolio-item'){
                $output .= '<div class="mkd-breadcrumbs"><div class="mkd-breadcrumbs-inner"><a itemprop="url" '.sparks_mikado_get_inline_style($bread_style).' href="/portfolio-item/">Portfolio</a>';

                if($_SERVER['REQUEST_URI'] != '/portfolio-item/'){
                    $output .= $delimiter;
                }               

                if(get_query_var('portfolio-category') !=''){
                    $catName = get_query_var('portfolio-category');
                    $output .= $before . $catName . $after;
                }

            }else{
                $output .= '<div class="mkd-breadcrumbs"><div class="mkd-breadcrumbs-inner"><a itemprop="url" '.sparks_mikado_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;
            }
                        

            if ( is_category() || sparks_mikado_is_product_category()) {
                ...
                $output .= $before . esc_html__('Search results for ', 'sparks') . '"' . get_search_query() . '"' . $after;

            } elseif ( is_day() ) {
                $output .= '<a itemprop="url" '.sparks_mikado_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a itemprop="url" '.sparks_mikado_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
                ...
                if ( get_post_type() != 'post' ) {                    
                    ...
                    if(get_post_type() == 'portfolio-item'){
                        $postId = get_the_ID();
                        $catPortfolio = get_the_terms( $postId, 'portfolio-category' ); 
                        $output .= '<a itemprop="url" '.sparks_mikado_get_inline_style($bread_style).' href="/portfolio-category/' . $catPortfolio[0]->slug . '">' . $catPortfolio[0]->name . '</a>' . $delimiter;
                    }
                ...
                $output .= '<a itemprop="url" '.sparks_mikado_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<a itemprop="url" '.sparks_mikado_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
                ...
                $output .= $before . esc_html__('Posts tagged ','sparks') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                global $authordata;
                $output .= $before . esc_html__('Articles posted by ','sparks') . $authordata->display_name . $after;
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'sparks') . ' ' . get_query_var('paged') . ")" . $after;
...
if(!function_exists('sparks_mikado_get_title_area_height')) {

    /**
     * Function that returns title height
     **/
    function sparks_mikado_get_title_area_height() {
        ...
        if(($meta_temp = get_post_meta($id, 'mkd_title_area_type_meta', true)) != '') {
            ...
            $title_type = sparks_mikado_options()->getOptionValue('title_area_type');
        }

        $title_height = 114; // default title height without header height
        ...
        } elseif(sparks_mikado_options()->getOptionValue('title_area_height') !== '') {
            $title_height = sparks_mikado_options()->getOptionValue('title_area_height');
        }

        return apply_filters('sparks_mikado_title_area_height', $title_height);
    }
}

if(!function_exists('sparks_mikado_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/

    function sparks_mikado_get_title_content_padding() {
        return apply_filters('sparks_mikado_title_content_padding', 0);
    }
}


if(!function_exists('sparks_mikado_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function sparks_mikado_title_area_height() {
        $id = sparks_mikado_get_page_id();
        $title_height_and_padding = array();
        $title_height          = sparks_mikado_get_title_area_height();
        $header_height_padding = sparks_mikado_get_title_content_padding();
        ...
        if(($meta_temp = get_post_meta($id, "mkd_title_area_background_image_responsive_meta", true)) != "") {
            ...
            $is_img_responsive = sparks_mikado_options()->getOptionValue('title_area_background_image_responsive');
        }

        if(($meta_temp = get_post_meta($id, "mkd_title_area_vertial_alignment_meta", true)) !== '') {
            $title_vertical_alignment = $meta_temp;
        }else{
            $title_vertical_alignment = sparks_mikado_options()->getOptionValue('title_area_vertial_alignment');;
        }
...
if(!function_exists('sparks_mikado_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     **/

    function sparks_mikado_title_area_background() {
        $id = sparks_mikado_get_page_id();
        $show_title_img = true;
        ...
        if(($meta_temp = get_post_meta($id, "mkd_title_area_background_color_meta", true)) != ""){
            ...
            $background_color = sparks_mikado_options()->getOptionValue('title_area_background_color');
        }
        ...
        if(($meta_temp = get_post_meta($id, "mkd_title_area_background_image_meta", true)) != ""){
            ...
            $background_image = sparks_mikado_options()->getOptionValue('title_area_background_image');
        }

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = sparks_mikado_get_image_dimensions($background_image);
